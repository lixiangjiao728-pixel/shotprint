...
模型原始证据最多保留48个连续时间段；结构结果按片长与均匀抽取的本机审计切点计算明确目标，300秒至少6段、最多24段，实际镜头更多时合并相邻段，仍须无空洞覆盖全片。前端合并最多 72 个均匀分布的证据镜头，始终保留首镜、末镜和中段，避免长片只传前 60 镜。最终作战书最多展示 24 个覆盖全片的关键执行镜头，并包含：
