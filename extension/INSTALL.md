...
1. 在镜谱首页点击“下载0.6.9扩展”，解压 `shotprint-extension-0.6.9.zip`，在 Chrome/Edge 的扩展管理页开启开发者模式，选择“加载已解压的扩展”。
2. 扩展详情应显示用户可见版本 0.6.9。如果同时加载了旧版镜谱扩展，请先移除或停用旧版。
...
4. 首页自检应显示扩展0.6.9和主采集通道可用；百炼联网状态单独核验。BrowserAct是可选高级兜底，未启动时不会覆盖扩展主采集的真实错误，也不会阻塞手动导入。
5. 采集上限为 200 条评论、最多 15 次有界滚动、90 秒；验证码、403、429和登录墙会立即停止，禁止绕过。页面适配器失效时才按“BrowserAct网络响应→BrowserAct DOM→手动导入”降级。
6. 校验页同时发布的 `shotprint-extension-0.6.9.zip.sha256`。扩展不会读取或上传 Cookie、头像、用户名、用户 ID；导出内容也不包含这些身份字段。
