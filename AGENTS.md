...
- 付费异步 POST 必须携带稳定的 `x-shotprint-task-id`；回执丢失后的重试复用同一任务，不能再次启动模型调用。
- 已配置外部后端时，连接失败保留原地址并允许重试，禁止静默切到未经验证的同源服务。生产更新只用原 Cloudflare 项目，用户验收地址为 `https://shotprint.xyz`。
- 扩展评论和媒体必须绑定任务与作品身份（含 B站分P）；切换作品不得复用旧结果。媒体地址与登录凭据不得回传主站；流媒体获取失败由用户确认录制或上传，不能冒充完整视频成功。
...
- 视频上传前置检查与正式分析必须共享 `VIDEO_ANALYSIS_BUDGET`；失败只结算已经尝试的模型调用，某次已尝试调用缺失 usage 时才按该次完整预留计费。
- 上传尝试与成功分析分开限流；取消、上传失败、预算拒绝和模型失败必须释放对应次数，不能让排错重试耗尽成功分析额度。

## 证据规则

- 没有真实视频证据时，锁定导演、制作、时间码和创作方案。
